...
class Integrity(val context: Context, val gcpKey: Long) {
  ...
  fun getToken(action: String, cb: (token: String, e: Exception?) -> Unit) {
    val tokenProvider = tokenProvider ?: run {
      cb("", IllegalStateException("Play Integrity API not warmed up"))
      return
    }

    val hash = createRequestHash(action)
    val request = StandardIntegrityTokenRequest
      .builder()
      .apply {
        setRequestHash(hash)
      }
